...
export class EventRepositoryMock implements IEventRepository {
  ...
  async findPaginated(_params: PaginationParams): Promise<PaginatedResult<EventEntity>> {
    return this.findPaginated_result
  }
  async countByStatusNotDeleted(_status: EventStatus): Promise<number> {
    return this.countByStatusNotDeleted_result
  }
  async countCompletedInYear(_year: number): Promise<number> {
    return this.countCompletedInYear_result
  }
  async countUpcomingInDays(_days: number): Promise<number> {
    return this.countUpcomingInDays_result
  }
