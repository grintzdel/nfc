...
export interface OrderEntityProps {
  id: string; userId: string; items: OrderItem[]; totalAmount: number
  status: OrderStatus; shippingAddress: string; createdAt: Date; updatedAt: Date
}
...
export class OrderEntity {
  ...
  static create(props: Partial<OrderEntityProps>): OrderEntity {
    ...
    return new OrderEntity({
      id: props.id ?? '', userId: props.userId, items, totalAmount,
      status: props.status ?? OrderStatus.PENDING, shippingAddress: props.shippingAddress,
      createdAt: props.createdAt ?? now, updatedAt: props.updatedAt ?? now,
    })
  }

  static fromProps(props: OrderEntityProps): OrderEntity { return new OrderEntity(props) }
  ...
  confirm(): this {
    ...
    this.props.status = OrderStatus.CONFIRMED; this.props.updatedAt = new Date(); return this
  }
  ship(): this {
    if (!this.isConfirmed()) throw new Error('Only confirmed orders can be shipped')
    this.props.status = OrderStatus.SHIPPED; this.props.updatedAt = new Date(); return this
  }
  deliver(): this {
    if (!this.isShipped()) throw new Error('Only shipped orders can be delivered')
    this.props.status = OrderStatus.DELIVERED; this.props.updatedAt = new Date(); return this
  }
  cancel(): this {
    ...
    this.props.status = OrderStatus.CANCELLED; this.props.updatedAt = new Date(); return this
  }

  toJSON(): OrderEntityProps { return { ...this.props } }
}
