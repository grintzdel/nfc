...
import { OnOrderConfirmed, UpdateOrderStatusUseCase } from './application/use-cases/update-order-status/update-order-status.use-case'
...
export function createOrderModule(
  ...
  const orderService = new OrderService(createOrderUseCase, getMyOrdersUseCase, getOrderByIdUseCase, getAllOrdersUseCase, updateOrderStatusUseCase)
  ...
  router.patch('/:id/status', authMiddleware, adminMiddleware, (req, res, next) => controller.updateStatus(req, res, next))
