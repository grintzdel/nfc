...
export class GetEventPageStatsUseCase {
  ...
  async execute(): Promise<AnalyticsDomainModel.EventPageStatsDto> {
    ...
    const allStatuses = [EventStatus.DRAFT, EventStatus.UPCOMING, EventStatus.IN_PROGRESS, EventStatus.COMPLETED, EventStatus.CANCELLED]

    const allEvents = await this.eventRepository.findAll()
    const totalEvents = allEvents.length

    const [thisMonthCount, lastMonthCount, upcomingIn30Days, inProgressCount, completedThisYear, cancelledThisYear] = await Promise.all([
